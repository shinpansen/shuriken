                     ...
                     Texture* textureWheel, Texture* textureCarpet)
{
    ...
    _wheelLeft.setOrigin(14.f/2.f, 11.f/2.f);
    _wheelRight.setOrigin(14.f/2.f, 11.f/2.f);
    _wheelLeft.setPosition(pos.x-(width*10/2-1), pos.y);
    _wheelRight.setPosition(pos.x+(width*10/2-1), pos.y);
    _animationWheelLeft = animation(&_wheelLeft, (float)fabs(speed)/8.f, 14, 11, 4, 2, 0);
    _animationWheelRight = animation(&_wheelRight, (float)fabs(speed)/8.f, 14, 11, 4, 2, 0);
    ...
    //Carpets
    int posX = pos.x - ((float)width/2)*10 + 5;
    for(int i=0 ; i<width ; i++)
    {
        ...
        _carpets[_carpets.size()-1]->setOrigin(10.f/2.f, 11.f/2.f);
        _carpets[_carpets.size()-1]->setPosition(posX + i*10, pos.y);
        *_animationCarpets[_animationCarpets.size()-1] =
            animation(_carpets[_carpets.size()-1], (float)fabs(speed)/2.f, 10, 11, 10, 2, 0);
        _animationCarpets[_animationCarpets.size()-1]->SetAnimation(speed < 0 ? 1 : 0, 0);
    }

    //Box2D - Collision
    _carpetBody = Box2DTools::CreateRectangleBox(_world, Vector2i(pos.x, pos.y-1), width*10, 12, 0.1, 0, 0, 9);
    _leftWheelBody = Box2DTools::CreateCircleBox(_world, Vector2i(pos.x-(width*10/2-1)-1,pos.y-1), 12, 0.1, 0, 9);
    _rightWheelBody = Box2DTools::CreateCircleBox(_world, Vector2i(pos.x+(width*10/2-1)+1,pos.y-1), 12, 0.1, 0, 9);
}
...
void speedwalk::Render()
{
    ...
    for(int i=0 ; i<_ninjas->size() ; i++)
    {
        ...
        b2Vec2 posLeft = b2Vec2(ninjaBody->GetPosition().x-(5/SCALE), ninjaBody->GetPosition().y+(8/SCALE));
        b2Vec2 posRight = b2Vec2(ninjaBody->GetPosition().x+(5/SCALE), ninjaBody->GetPosition().y+(8/SCALE));
        b2Vec2 posMiddle = b2Vec2(ninjaBody->GetPosition().x, ninjaBody->GetPosition().y+(8/SCALE));
