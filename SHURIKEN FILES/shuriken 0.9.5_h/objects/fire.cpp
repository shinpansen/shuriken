...
fire::fire(vector<hero*>* ninjas, Vector2i pos, b2World* world, Texture* textureFire)
{
    _ninjas = ninjas;
    _world = world;
    _body = Box2DTools::CreateRectangleBox(_world, pos, 8, 14, 0.1, 0, 0, 8);
    _bodyBottom = Box2DTools::CreateRectangleBox(_world, Vector2i(pos.x, pos.y+9), 10, 10, 0.1, 0, 0, 2);
    _initPos = pos;
    ...
    _fire.setOrigin(9, 20);
    _animation = animation(&_fire, 0.1, 18, 25, 3, 1, 0);
...
void fire::Render()
{
    ...
    for(int i=0 ; i<_ninjas->size() ; i++)
    {
        ...
        b2Vec2 posLeft = b2Vec2(ninjaBody->GetPosition().x-(5.f/SCALE), ninjaBody->GetPosition().y);
        b2Vec2 posRight = b2Vec2(ninjaBody->GetPosition().x+(5.f/SCALE), ninjaBody->GetPosition().y);
        b2Vec2 posTop = b2Vec2(ninjaBody->GetPosition().x, ninjaBody->GetPosition().y-(8.f/SCALE));
        b2Vec2 posBottom = b2Vec2(ninjaBody->GetPosition().x, ninjaBody->GetPosition().y+(8.f/SCALE));
           ...
           _body->GetFixtureList()[0].TestPoint(posBottom))
        {
            ...
            (*_ninjas)[i]->SetStunned(Vector2f(signe*4.f, -3.f), 10);
        }
    }
...
void fire::Display(RenderWindow* window)
{
    if(_body != NULL)
        Render();
    _fire.setPosition(_position.x, _position.y+8);
    window->draw(_fire);
}
