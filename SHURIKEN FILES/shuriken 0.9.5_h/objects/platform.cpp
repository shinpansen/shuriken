...
{
    _world = world;
    _body = Box2DTools::CreateRectangleBox(_world, pos, 30, 8, 0.1, 0, 0, enemySensor ? 2 : 11);
    _initPos = pos;
...
void platform::Render()
{
    ...
    _body->SetLinearVelocity(b2Vec2(_direction.x*0.8f, _direction.y*0.8f));
...
{
    Render();
    _platform.setPosition(_position.x, _position.y-3);
