...
projectile::projectile(b2World* world, effects* effectBox, sound* soundBox)
{
    ...
    if (!_projectile2Texture.loadFromFile(_files.projectile2Tile))
        cout << "Unable to find projectile2 texture '" << _files.projectile2Tile << "'" << endl;
                               ...
                               int numAnimation, int angle, Vector2f speed, int type)
{
    ...
    try
    {
        ...
        _lifeOfProjectile.push_back(type <= 0 ? 100 : type != 4 ? 300 : 666);

        //Settings
        _tiles[_tiles.size()-1]->setTexture(type <= 3 ? _projectileTexture : _projectile2Texture);
        ...
        //Type of hit box
        if(type <= 0)
            _projectileBodies.push_back(Box2DTools::CreateCircleBox(_world, pos, 8, 0.5, 1, 5));
        else if(type <= 2)
            _projectileBodies.push_back(Box2DTools::CreateRectangleBox(_world, pos, 7, 5, 0.5, 1, 0, 6));
        else if(type <= 3)
            _projectileBodies.push_back(Box2DTools::CreateRectangleBox(_world, pos, 9, 8, 0.5, 1, 0, 6));
        else
            _projectileBodies.push_back(Box2DTools::CreateCircleBox(_world, pos, 12, 0.5, 1, 6));

        _projectileBodies[_projectileBodies.size()-1]->SetLinearVelocity(b2Vec2(speed.x, speed.y));
        _projectileBodies[_projectileBodies.size()-1]->ApplyForce(b2Vec2(0.f,-GRAVITY*_projectileBodies[_projectileBodies.size()-1]->GetMass()), _projectileBodies[_projectileBodies.size()-1]->GetWorldCenter(), false);
        ...
        *_animations[_animations.size()-1] =
        ...
        if(type <= 3)
            ...
            _animations[_animations.size()-1]->SetAnimation(type-4, 0);
    }
    catch(exception e)
    {
...
void projectile::DestroyProjectile(int i, bool showEffect, bool playSound)
{
    if(i > 0 && i < _projectileBodies.size())
    {
        ...
        if(showEffect)
            _effectBox->AddEffect(Vector2i(x,y), 0.3, 25, 25, 6, 0, false);
        if(playSound)
...
void projectile::DestroyLine(int posY)
{
    ...
    for(int i=0 ; i<_enemies->size() ; i++)
        if(fabs(posY - (*_enemies)[i]->GetPosition().y) < 20)
    ...
    while(ProjectileInLine(posY))
    {
        for(int i=0 ; i<_projectileBodies.size() ; i++)
        {
            if(_projectileBodies[i] == NULL)
                continue;
            if(fabs(posY - _projectileBodies[i]->GetPosition().y*SCALE) < 20)
            {
...
{
    for(int i=0 ; i<_projectileBodies.size() ; i++)
        if(_projectileBodies[i] != NULL && fabs(posY - _projectileBodies[i]->GetPosition().y*SCALE) < 20)
            return true;
    return false;
}
        ...
        for (b2Contact* contact = _world->GetContactList() ; contact ; contact = contact->GetNext())
        {
                    ...
                    contact->GetFixtureB()->GetBody() == projectile)
            {
                for(int i=0 ; i<_enemies->size() ; i++)
                {
                    ...
                    {
                        int signe = _speedOfProjectile[projectileNumber].x > 0 ? -1 : 1;
                        float stunRecoil = (_speedOfProjectile[projectileNumber].x != 0)*-30.f*signe;
                        float dommageBase = fabs(_speedOfProjectile[projectileNumber].x) > 8 ? 2.f : 1.f;
                        DestroyProjectile(projectileNumber, true, true);
                        (*_enemies)[i]->SetDommages(dommageBase*_dommagesRatio, stunRecoil);
                    }
                }
                ...
                for(int i=0 ; i<_ninjas->size() ; i++)
                {
                    if((*_ninjas)[i]->isAHero(contact->GetFixtureA()->GetBody(), contact->GetFixtureB()->GetBody()))
                    {
                        ...
                        DestroyProjectile(projectileNumber, true, true);
                        (*_ninjas)[i]->SetStunned(Vector2f(signe*5.f, -4.f), 15);
                    }
                }
            }
        }
...
bool projectile::AboveAProjectile(Vector2f pos, Vector2f* projectilePos)
{
    for(int i=1 ; i<_projectileBodies.size() ; i++)
    {
        if(_projectileBodies[i] != NULL)
        {
            ...
            if(category == PROJECTILE_ENTITY && pos.y < y && fabs(pos.y - y) < 25 && fabs(pos.x - x) < 8)
            {
                *projectilePos = Vector2f(x+_speedOfProjectile[i].x*5, y-5); // Position in 5 frames
                return true;
            }
        }
    }
...
void projectile::RenderProjectiles(float deltaTime)
{
    try
    {
        ...
        for(int i=1 ; i<_tiles.size() ; i++)
        {
            ...
            if(_lifeOfProjectile[i] != 666) // Boss bomb
                _projectileBodies[i]->ApplyForce(b2Vec2(0.f,-GRAVITY*_projectileBodies[i]->GetMass()), _projectileBodies[i]->GetWorldCenter(), false); //Remove gravity
            else
                _projectileBodies[i]->ApplyForce(b2Vec2(0.f,4*GRAVITY*_projectileBodies[i]->GetMass()), _projectileBodies[i]->GetWorldCenter(), false); //Increase gravity
        ...
        {
            _lifeOfProjectile[i] -= _lifeOfProjectile[i] != 666 ? 1 : 0;
