...
{
    int x = ((view->getCenter().x  - ((int)view->getCenter().x % 16)) - 400) + (pos.x - (pos.x % 16));
    int y = ((view->getCenter().y  - ((int)view->getCenter().y % 16)) - 300) + (pos.y - (pos.y % 16));
                            ...
                            bool* mousePressed, int* x1, int* x2, int* y1, int* y2, int type, vector<int>* collisionType)
{
    ...
    else if(Mouse::isButtonPressed(Mouse::Left))
    {
        ...
        *y2 = getRect(Mouse::getPosition(*window)).y;
        ...
        //Draw current shape
        IntRect rect = calcRect(*x1+view->getCenter().x-400, *x2+view->getCenter().x-400,
                                *y1+view->getCenter().y-300, *y2+view->getCenter().y-300);
        RectangleShape shape;
        Vector2i pos = getRect(Vector2i(*x1+view->getCenter().x-400, *y1+view->getCenter().y-300));
...
RectangleShape tools::getShape(IntRect* rect, View* view, int x1, int x2, int y1, int y2)
{
    ...
    shape.setOrigin(0,0);
    int topX = view->getCenter().x-400;
    int topY = view->getCenter().y-300;
...
void tools::drawCursor(RenderWindow* window, View* view, Color color)
{
    ...
    Vector2i pos2 = getRect(Vector2i(pos.x+view->getCenter().x-400, pos.y+view->getCenter().y-300));
