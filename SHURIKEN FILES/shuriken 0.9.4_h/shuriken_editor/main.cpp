...
{
    //Window
    RenderWindow window(VideoMode(280, 192, 32), "shuriken editor");//, Style::Fullscreen);
    window.setVerticalSyncEnabled(true);
    window.setFramerateLimit(60);

    //View
    View view;
    view.setSize(280,192);
    view.setCenter(140,96);
    ...
    //Map
    effects effectBox;
    b2World* world = new b2World(b2Vec2(0.f, 50.f));
    ...
    while (window.isOpen())
    {
        ...
        Vector2f mousePos = mouse_pos - view.getCenter();
        mousePos = Vector2f(mousePos.x + 140, mousePos.y + 96);

        //Delete sprite
        if(Mouse::isButtonPressed(Mouse::Right))
        {
            if(mode == 0)
                tools::eraseSprite(&maps._layer0Tiles, &view,Vector2i(mousePos.x, mousePos.y));
            else if(mode == 1)
                tools::eraseSprite(&maps._layer1Tiles, &view,Vector2i(mousePos.x, mousePos.y));
            else if(mode == 2 || mode == 3)
                tools::eraseCollision(&shapes, &view,Vector2i(mousePos.x, mousePos.y), &collisionType);
            else if(mode == 4)
                tools::eraseCollision(&ladderShapes, &view,Vector2i(mousePos.x, mousePos.y));
            else if(mode == 5)
                tools::eraseTree(&maps._treeList, &view,Vector2i(mousePos.x, mousePos.y));
        }
        ...
        else //Display sprites
        {
            ...
            if(!Mouse::isButtonPressed(Mouse::Left))
                tools::moveView(&window, &view,Vector2i(mousePos.x, mousePos.y));

            if(mode == 0 || mode == 1)
            {
                //Add Sprite
                window.setMouseCursorVisible(true);
                tools::drawSprite(&window, &sprite, &view,Vector2i(mousePos.x, mousePos.y));
            ...
            else if (mode == 5)
            {
                ...
                Vector2f pos = Vector2f(mousePos.x, mousePos.y);
                pos.x += view.getCenter().x - 140;
                pos.y += view.getCenter().y - 96;
                window.setMouseCursorVisible(true);
