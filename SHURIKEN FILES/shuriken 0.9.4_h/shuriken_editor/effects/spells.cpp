                      ...
                      int numAnimation, int angle, Vector2f speed, Color color)
{
    try
    {
        ...
        _spellsBodies.push_back(Box2DTools::CreateRectangleBox(_world, pos, 17, 17, 0.5, 1, 0, 4));
    ...
    try
    {
        ...
        for (b2Contact* contact = _world->GetContactList() ; contact ; contact = contact->GetNext())
        {
            ...
            //Contact with wizard
                     ...
                     contact->GetFixtureB()->GetBody() == spell) &&
                    (contact->GetFixtureA()->GetBody() == (*_wizards)[i]->GetBody() ||
                     contact->GetFixtureB()->GetBody() == (*_wizards)[i]->GetBody()))
            {
                //{Counteracting
                if(((*_wizards)[i]->isCounteracting() == 1 &&
                     spell->GetPosition().x >= (*_wizards)[i]->GetBody()->GetPosition().x) ||
                    ...
                    ((*_wizards)[i]->isCounteracting() != 0 && fabs((*speedOfSpell).x) <= 0.5))
                {
                    ...
                    {
                        if(fabs((*speedOfSpell).x) > 0.5 && spell->GetPosition().x >= (*_wizards)[i]->GetBody()->GetPosition().x)
                        {
                        ...
                        else if(fabs((*speedOfSpell).x) > 0.5 && spell->GetPosition().x < (*_wizards)[i]->GetBody()->GetPosition().x)
                        {
                            spell->SetTransform(b2Vec2(spell->GetPosition().x-25/SCALE, spell->GetPosition().y), spell->GetAngle());
                   ...
                   spell->GetPosition().x > (*_wizards)[i]->GetBody()->GetPosition().x))
                {
                    ...
                    if(fabs(y) <= 0.1)
                        y = -2;
                    (*_wizards)[i]->SetStunned(Vector2f(x, y), 15);
                    DestroySpell(spellNumber, true, true);
                    break;
                }
            }
...
void spells::RenderSpells()
{
    try
    {
        ...
        for(int i=1 ; i<_tiles.size() ; i++)
        {
            ...
            _effectBox->AddStar(_lightOfSpells[i]->getFillColor(), Vector2f(x, y), 1);
