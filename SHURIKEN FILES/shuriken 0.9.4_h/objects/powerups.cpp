...
static const int POWERUP_SIZE = 11;
...
void powerups::DeletePowerUp(int i)
{
    if(i >= 0 && i < _powerups.size())
    {
        ...
        _effectBox->AddEffect(pos, 0.2, 25, 25, 6, 4, false);
    }
}
...
void powerups::Render()
{
    for(int i=0; i<_powerups.size() ; i++)
    {
        ...
        b2Vec2 point = b2Vec2(_powerups[i]->getPosition().x/SCALE, (_powerups[i]->getPosition().y+3.5)/SCALE);
        if(!Box2DTools::TestCollisionPoint(_world, point, true, new int))
            _powerups[i]->move(0,0.5);
        ...
        //Collision with hero
        if(fabs((*_ninjas)[0]->GetPosition().x - _powerups[i]->getPosition().x) < (POWERUP_SIZE/2)+5 &&
           fabs((*_ninjas)[0]->GetPosition().y - _powerups[i]->getPosition().y) < (POWERUP_SIZE/2)+8)
        {
