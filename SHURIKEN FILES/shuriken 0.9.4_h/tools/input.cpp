...
input::input(int player, int deadZone, RenderWindow* window)
{
    ...
    else if(player == 3)
    {
        ...
        _KEY_SPECIAL = Keyboard::SemiColon;
        _KEY_UP = Keyboard::I;
    ...
    if(_player >= 0)
    {
        ...
        if(keysFromXml.size() >= 8)
        {
            ...
            _KEY_SPECIAL = (Keyboard::Key)atoi(keysFromXml[2].c_str());
...
bool input::isKeyPressed(string action)
{
    if(_player >= 0)
    {
        switch(ResolveOption(action))
        {
            ...
            case SPECIAL:
                return Joystick::isButtonPressed(_player, _JOY_SPECIAL) ||
                       Keyboard::isKeyPressed(_KEY_SPECIAL);
            case UP:
...
string input::GetKeyNameInput(const sf::Keyboard::Key key)
{
    switch(key)
    {
        default:
...
string input::GetKeyString(Options keyType)
{
    switch(keyType)
    {
        case JUMP:
            return GetKeyNameInput(_KEY_JUMP);
        case ATTACK:
            return GetKeyNameInput(_KEY_ATTACK);
        case SPECIAL:
            return GetKeyNameInput(_KEY_SPECIAL);
        case UP:
            return GetKeyNameInput(_KEY_UP);
        case DOWN:
            return GetKeyNameInput(_KEY_DOWN);
        case LEFT:
            return GetKeyNameInput(_KEY_LEFT);
        case RIGHT:
            return GetKeyNameInput(_KEY_RIGHT);
        default:
            return GetKeyNameInput(_KEY_JUMP);
    }
}
