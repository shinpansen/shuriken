...
static const int ENEMY_A_LIFE = 10;
static const int ENEMY_B_LIFE = 6;
static const int ENEMY_C_LIFE = 10;
static const int ENEMY_D_LIFE = 15;
          ...
          Texture* textureParachute)
{
    ...
    _ignorePlatform = _type != 2 ? false : rand()%3 > 0 && pos.x < 400 ? true : false;
    _attacking = 0;
...
void enemy::UpdateVariables()
{
    ...
    if(_type == 2)
    {
        ...
        if(collisionType == 2 && (_position.y < 150 || (_ignorePlatform && !_animation.GetReversed() && _position.y > 450)))
            _jumpingTimer = 0;

        //Jumping timer
        _jumpingTimer -= _jumpingTimer > 0 ? 1 : 0;
        if(_jumpingTimer == 0 && (collisionType != 2 || _position.y < 150 || (_ignorePlatform && !_animation.GetReversed() && _position.y > 450)))
        {
            _jumpingTimer = JUMPING_TIMER;
    ...
    if(_life <= 0 && _body != NULL && _body->IsActive()) //Killing enemy if running out of life
    {
        ...
        _effectBox->AddEffect(Vector2i(_position.x, _position.y-25), 0.20, 75, 75, 6, 2, _animation.GetReversed(), 1);
...
void enemy::PerformEffects()
{
    ...
    {
        _spriteClone.setPosition(840+_position.x, _position.y);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2((840+_position.x)/SCALE, _position.y/SCALE),0);
    }
    else if(_position.x+_hitboxWidth >= 840)
    {
        _spriteClone.setPosition(_position.x-840, _position.y);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2((_position.x-840)/SCALE, _position.y/SCALE),0);
    }

    //Disabling collision of clone
    if(_position.x-_hitboxWidth > 0 && _position.x+_hitboxWidth < 840 && _position.y-30 > 0 && _position.y+30 < 576)
    {
    ...
    int enemySpeed, projectileSpeed, projectileType;
    ...
    switch(_type)
    {
        case 0:
            enemySpeed = 2;
            projectileSpeed = 5;
            projectilePos = Vector2i(55,12);
            projectileType = 1;
            break;
        case 1:
            enemySpeed = 2.5;
            projectileSpeed = 5;
            projectilePos = Vector2i(55,8);
            projectileType = 2;
            break;
        case 2:
            enemySpeed = !_onTheFloor ? 7 : 0;
            break;
        case 3:
            enemySpeed = 2;
            projectileSpeed = 5;
            projectilePos = Vector2i(80,29);
            projectileType = 3;
            break;
    }
    ...
    if(_type != 2 && _attackingCooldown == 0 && _turning == 0)
    {
        ...
        _projectileBox->AddProjectile(projectilePosFinal, 0.2, 39, 39, 2, 2, angle, Vector2f(signe*projectileSpeed,0.f), projectileType);
        _soundBox->PlaySound(SND_SHOOT2, false);
    }

    //Changing direction if something in front
    int spaceInFront = _hitboxWidth/2 + 5;// _type != 2 ? 45 : 25;
    b2Vec2 pointA = b2Vec2(_body->GetPosition().x+(signe*spaceInFront/SCALE), _body->GetPosition().y-(15/SCALE));
    b2Vec2 pointB = b2Vec2(_body->GetPosition().x+(signe*spaceInFront/SCALE), _body->GetPosition().y-(5/SCALE));
    int collisionPoint;
    b2Body* bodyTest;
...
void enemy::Display(RenderWindow* window, float deltaTime)
{
    ...
    if(_position.x-43 <= 0 || _position.x+43 >= 840)
    {
