...
b2Body* Box2DTools::CreateRectangleBox(b2World* world, Vector2i pos, int width, int height, float friction, float density, float angle, int category)
{
    ...
    //Category - Mask : who am I ? what/who can I collide with ?
    fixtureDef->filter.categoryBits = Entity(category);
    if(category == 1)
        fixtureDef->filter.maskBits = GROUND_ENTITY|HERO_ENTITY|ENEMY_ENTITY|SHURIKEN_ENTITY|PROJECTILE_ENTITY;
    else if(category == 2)
...
{
    //TODO - Handle code duplication with CreateRectangleBox method
    b2BodyDef* bodyDef = new b2BodyDef;
    bodyDef->position = b2Vec2(pos.x/SCALE, pos.y/SCALE);
    bodyDef->type = GetType(category);
    b2Body* body = world->CreateBody(bodyDef);
...
void Box2DTools::ManageBodyLoop(b2World* world)
{
    ...
    for (b2Body* BodyIterator = world->GetBodyList(); BodyIterator != 0; BodyIterator = BodyIterator->GetNext())
    {
        ...
        {
            if(BodyIterator->GetPosition().x*SCALE > 840)
                ...
                BodyIterator->SetTransform(b2Vec2(840/SCALE,BodyIterator->GetPosition().y), BodyIterator->GetAngle());
