...
menuList::menuList(vector<string> items, Vector2f pos, input* input, sound* soundBox, string alignment)
{
    ...
    for(int i=0 ; i<_items.size() ; i++)
    {
        ...
        _texts[_texts.size()-1]->setCharacterSize(6);
    ...
    {
        if(alignment == "LEFT")
            _texts[i]->setPosition(pos.x - _maxTextWidth/2, topPos+8+(i*(textRect.height+5)));
        else if(alignment == "RIGHT")
            _texts[i]->setPosition(pos.x + _maxTextWidth/2, topPos+8+(i*(textRect.height+5)));
        else
            _texts[i]->setPosition(pos.x, topPos+8+(i*(textRect.height+5)));
    }
