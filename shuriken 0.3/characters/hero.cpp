...
static const float WALKING_SPEED = 7.f;
static const float RUNNING_SPEED = 7.f;
static const float FALLING_SPEED = 10.f;
           ...
           sound* soundBox, scoreManager* scoreManager, int life, settings* settings)
{
    ...
    //Box2D
    _world = world;
    _body = Box2DTools::CreateRectangleBox(_world, pos, 7, 12, 0.1, 0, 0, 3);
    _bodyClone = Box2DTools::CreateRectangleBox(_world, pos, 7, 12, 0.1, 0, 0, 3);
...
void hero::UpdateVariables()
{
    ...
    _onTheFloor = false;
    _isCrushed = false;
    for(double i=_position.x-3.6 ; i<_position.x+3.6 ; i+=0.1)
    {
        b2Body* body;
        if(Box2DTools::TestCollisionPoint(_world, b2Vec2(i/SCALE, (_position.y+8)/SCALE), true, &_collisionType, &body))
            _onTheFloor = true;
        if(Box2DTools::TestCollisionPoint(_world, b2Vec2(i/SCALE, (_position.y-8)/SCALE), false, &colType, &body))
            _isCrushed = true;
    }
    if(!_onTheFloor)
        _collisionType = -1;
    ...
    {
        b2Vec2 posLeft = b2Vec2(_body->GetPosition().x-(4/SCALE), _body->GetPosition().y-(5/SCALE));
        b2Vec2 posRight = b2Vec2(_body->GetPosition().x+(4/SCALE), _body->GetPosition().y-(5/SCALE));
        ...
        if(!Box2DTools::TestCollisionPoint(_world, posGround, true, &_collisionType))
        {
            ...
            {
                _body->SetLinearVelocity(b2Vec2(0,0));
                _body->SetTransform(b2Vec2(_hookedPoint.x + ((_hooked > 0 ? 4 : -4)/SCALE), _hookedPoint.y+(5/SCALE)), 0);
                _speed = Vector2f(_body->GetLinearVelocity().x, _body->GetLinearVelocity().y);
    ...
    if(_life == 0 && _stunned == 1) //Killing hero if running out of life
    {
        ...
        _effectBox->AddEffect(Vector2i(_position.x, _position.y-8), 0.20, 25, 25, 6, 1, _animation.GetReversed(), 1);
...
void hero::PerformEffects()
{
    ...
    if(_position.x-4 <= 0)
    {
        _spriteClone.setPosition(NATIVE_WIDTH+_position.x, _position.y+17);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2((NATIVE_WIDTH+_position.x)/SCALE, _position.y/SCALE),0);
    }
    else if(_position.x+4 >= NATIVE_WIDTH)
    {
        _spriteClone.setPosition(_position.x-NATIVE_WIDTH, _position.y+17);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2((_position.x-NATIVE_WIDTH)/SCALE, _position.y/SCALE),0);
    }
    else if(_position.y-10 <= 0)
    {
        _spriteClone.setPosition(_position.x, _position.y+NATIVE_HEIGHT+28);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2(_position.x/SCALE, (_position.y+NATIVE_HEIGHT)/SCALE),0);
    }
    else if(_position.y+10 >= NATIVE_HEIGHT)
    {
        _spriteClone.setPosition(_position.x, _position.y-NATIVE_HEIGHT+28);
        _bodyClone->SetActive(true);
        _bodyClone->SetTransform(b2Vec2(_position.x/SCALE, (_position.y-+NATIVE_HEIGHT)/SCALE),0);
    ...
    if(_position.x-4 > 0 && _position.x+4 < NATIVE_WIDTH && _position.y-10 > 0 && _position.y+10 < NATIVE_HEIGHT)
    {
...
{
    b2Vec2 posLeft = b2Vec2(_body->GetPosition().x-(4/SCALE), _body->GetPosition().y);
    b2Vec2 posRight = b2Vec2(_body->GetPosition().x+(4/SCALE), _body->GetPosition().y);
    b2Vec2 posTop = b2Vec2(_body->GetPosition().x, _body->GetPosition().y-(10/SCALE));
    b2Vec2 posBottom = b2Vec2(_body->GetPosition().x, _body->GetPosition().y+(10/SCALE));
...
void hero::Move()
{
       ...
       (_charging == 0 || _charging > 15))
    {
        ...
        int localSpeed = fabs(_speed.x) < 3.5 ? signe*4 : _speed.x + signe;
        int randY = rand()%4-1;
        Vector2i projectilePos;
        if(_input->isKeyPressed("UP"))
            projectilePos = Vector2i(position.x -signe*3, position.y-7);
        else
            projectilePos = Vector2i(position.x + localSpeed, position.y-randY-1);
        ...
        if(_doubleShuriken && !_input->isKeyPressed("UP"))
            _projectileBox->AddProjectile(Vector2i(position.x - localSpeed, position.y-randY-1), 0.3, 13, 13, 4, 0, 0, -projectileSpeed, projectileType);
    ...
    else
    {
        ...
        {
            _world->DestroyBody(_body);
            _body = Box2DTools::CreateRectangleBox(_world, Vector2i(_position.x,_position.y-7), 7, 12, 0.1, 0, 0, 3);
        }
        _isDown = false;
    }
...
void hero::AnimeSprite()
{
    ...
    _sprite.setPosition(_position.x+2, _position.y+8-(_isDown*3));

    //Special attack animation
    if(_animation.GetReversed())
        _bladeRectangle.setPosition(36*_specialAttackFrame-650, _position.y);
    else
        _bladeRectangle.setPosition(-NATIVE_WIDTH+36*(8-_specialAttackFrame), _position.y);
    ...
    if(_stunned == 0 && _invincibilityFrame == 0 && _life > 0)
    {
        ...
        if(_projectileBox->GetDommagesRatio() > 1)
        {
            ...
            _effectBox->AddText(xmlReader::getTranslationText("other", _settings->_language, "powerup2") + stream.str(),
                                Vector2f(_position.x, _position.y-8), Vector2f(0,-1), 60);
        }
    }
}
...
void hero::DrawLife(RenderWindow* window)
{
    ...
    for(int i=0 ; i<_maxLife ; i++)
    {
        ...
        shape.setPosition(_lifeText.getLocalBounds().width+5+i*8,8);
        window->draw(shape);
    }
    ...
    for(int i=0 ; i<3 ; i++)
    {
        _livesSprites[i].setPosition(_lifeText.getLocalBounds().width+33 + i*12, 4);
        if(i+1 <= _lives)
...
void hero::Display(RenderWindow* window, float deltaTime)
{
    ...
    //Hero sprite
    window->draw(_sprite);
    if(_position.x-4 <= 0 || _position.x+4 >= NATIVE_WIDTH)
    {
