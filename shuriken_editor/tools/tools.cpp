...
{
    int x = ((view->getCenter().x  - ((int)view->getCenter().x % 16)) - 420) + (pos.x - (pos.x % 16));
    int y = ((view->getCenter().y  - ((int)view->getCenter().y % 16)) - 288) + (pos.y - (pos.y % 16));
...
void tools::eraseSprite(vector<Sprite*>* sprites, View* view, Vector2i mouse)
{
    ...
    pos.x += view->getCenter().x - 420;
    pos.y += view->getCenter().y - 288;
                            ...
                            bool* mousePressed, int* x1, int* x2, int* y1, int* y2, int type, vector<int>* collisionType)
{
    ...
    else if(Mouse::isButtonPressed(Mouse::Left))
    {
        ...
        *y2 = getRect(Mouse::getPosition(*window)).y;
        ...
        //Draw current shape
        IntRect rect = calcRect(*x1+view->getCenter().x-420, *x2+view->getCenter().x-420,
                                *y1+view->getCenter().y-288, *y2+view->getCenter().y-288);
        RectangleShape shape;
        Vector2i pos = getRect(Vector2i(*x1+view->getCenter().x-420, *y1+view->getCenter().y-288));
...
{
    int x = (pos.x - (pos.x % 16));
    int y = (pos.y - (pos.y % 16));
    return Vector2i(x,y);
}
...
RectangleShape tools::getShape(IntRect* rect, View* view, int x1, int x2, int y1, int y2)
{
    ...
    shape.setOrigin(0,0);
    int topX = view->getCenter().x-420;
    int topY = view->getCenter().y-288;
...
void tools::manageMode(bool* directionPressed, int* mode, Text* text, RenderWindow* window)
{
    ...
    if(*mode == 0)
        ...
        text->setString("TREE");
    text->setPosition(window->getView().getCenter().x-395, window->getView().getCenter().y+250);
    window->draw(*text);
}
...
void tools::drawCursor(RenderWindow* window, View* view, Color color)
{
    ...
    Vector2i pos2 = getRect(Vector2i(pos.x+view->getCenter().x-420, pos.y+view->getCenter().y-288));
    shapeCursor.setPosition(pos2.x, pos2.y);
    shapeCursor.setSize(Vector2f(16,16));
